$(document).ready(function () {
	...
	var id = getvar("id");
	var urls = base_url + "index.php/world/read-world/" + id + "/" + localStorage.lat + "/" + localStorage.long + "/?callback=?";
		...
		success: function (data) {
			...
			if (data['status'] == "OK") {
				...
				<div class="blog-fullwidth animated fadein delay-1">
					<div class="blog-image"><img src="${data['detailgambar']}" alt="" /></div>
					<div class="tabs"><div class="tab tab-active" onClick="javascript:fade('funcommunity-read.html?id=${id}');">Profile</div>
					<div class="tab" onClick="javascript:fade('lokasiComunity.html?id=${id}');">Lokasi</div>
					<div class="tab" onClick="javascript:fade('belivoucher.html?id=${id}');">Tiket</div>
				</div>
					<div class="blog-preview p-20"><h4><strong>${data["detailnama"]}</strong></h4></p>
					<p><br>${data["detaillengkap"]}</p>
				`;
