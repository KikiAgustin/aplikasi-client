$(document).ready(function () {
  ...
  if(localStorage.cekref == 1) {
    localStorage.cekref = 2;
    setTimeout(function(){
      ...
      if (data['status'] == "OK") {
        ...
        for (var key in slides) {
          if (slides.hasOwnProperty(key)) {
            if (slides[key]["jenis"] == "1") dataslide += '<div class="swiper-slide"   onclick="javascript:fade(\'' + slides[key]["url"] + '\')">';
      ...
      if (data['status'] == "OK") {
        ...
        for (var key in dl) {
          if (dl.hasOwnProperty(key)) {
            datalist += '<div class="promo-depan prom"><div class="promo-depan-image"><a onclick="javascript:fade(\'promo-read.html?action=read&id=' + dl[key]["id"] + '\')"><img src="' + dl[key]["gambar"] + '" alt="" /></a></div>';
            datalist += '<div class="promo-depan-preview p-20"><strong><a onclick="javascript:fade(\'promo-read.html?action=read&id=' + dl[key]["id"] + '\')">' + dl[key]["nama"] + '</a></strong><br>' + dl[key]["ringkas"] + '</p></div>';
            datalist += '</div>';
          }
        }
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == "OK") {
        ...
        for (var key in dl) {
          if (dl.hasOwnProperty(key)) {
            datalist += '<div class="promo-depan"><div class="promo-depan-image"><a onclick="javascript:fade(\'blog-read.html?action=read&id=' + dl[key]["id"] + '\')"><img src="' + dl[key]["gambar"] + '"/></a></div>';
            datalist += '<div class="promo-depan-preview p-20"><strong><a onclick="javascript:fade(\'blog-read.html?action=read&id=' + dl[key]["id"] + '\')">' + dl[key]["nama"] + '</a></strong><br>' + dl[key]["ringkas"] + '</p></div>';
            datalist += '</div>';
          }
        }
