                            ...
                            if (datas['mitraid'] > 0 && datas['mitraid_until'] >= dt) {
                                ...
                                for (var key in dl) {
                                    if (dl.hasOwnProperty(key)) {
                                        datalist += '<a class="product-sec waves-effect waves-ripple animated fadein" onclikc="javascript:fade(\'chat-group.html?action=read&chatid=' + dl[key]["id"] + '&userid=' + localStorage.userid + '\')"><strong><i class="fal fa-users"></i>  ' + dl[key]["nama"] + '</strong> <i class="fal fa-arrow-right right"></i></a>';
                                        /* var submenu = dl[key]["sub"];
                
                                        for (var subkey in submenu) {
                                            if (submenu.hasOwnProperty(subkey)) {
                
                                                datalist += '<a  class="product-sec waves-effect waves-ripple animated fadein" onclikc="javascript:fade(\'konsul-kategori-konsultan.html?action=read&subid=' + submenu[subkey]["subid"] + '&secid=' + dl[key]["secid"] + '\')">' + submenu[subkey]["namasub"] + ' <i class="fal fa-arrow-right right"></i></a>';
                                            }
                                        } */
                                    }
                                }
